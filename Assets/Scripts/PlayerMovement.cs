...
public class Movement : MonoBehaviour
{
    ...
    [Tooltip("Yaw speed (treat as degrees per second-ish when multiplied by deltaTime)")]
    [SerializeField] float yawSpeed = 10f;
    [Tooltip("Pitch speed (treat as degrees per second-ish when multiplied by deltaTime)")]
    ...
    private void Update()
    {
        switch (jumpStyle)
        {
            ...
            case JumpStyle.Charged:
                ...
                if (onGround && Keyboard.current.spaceKey.wasReleasedThisFrame)
                {
        ...
        // Mouse look (scaled by deltaTime so it doesn't feel insanely fast vs old Input.GetAxis)
        Vector2 mouseDelta = Mouse.current.delta.ReadValue();
        yaw += mouseDelta.x * yawSpeed * Time.deltaTime;
        pitch = Mathf.Clamp(pitch - mouseDelta.y * pitchSpeed * Time.deltaTime, pitchMinimum, pitchMaximum);
        ...
        desiredVelocity = new Vector3(horizontal, 0, vertical) * walkSpeed;
    }
